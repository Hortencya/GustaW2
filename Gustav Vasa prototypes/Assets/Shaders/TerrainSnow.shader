Shader "Custom/TerrainSnow" {
	Properties{
		...
		SubShader{
		Tags{
			"SplatCount" = "1"// assign the amount of splatmaps this shader uses(1) for snow displacement
			"Queue" = "Geometry-100" // specifies placement in the renderque
			"RenderType" = "Opaque"	
		}
		LOD 200
		...
		struct appdata
	{
		...
		//float4 tangent : TANGENT; //in this testhader I make use of the grid used to generate terrain instead		
	...
	uniform sampler2D _Splat; // splatmap used for displacement
	...
	struct Input {
		// below are the input coordinates used by the shader in order to 
		float2 uv_GroundTex: TEXCOORD0;
		float2 uv_SnowTex: TEXCOORD1;
